...
export default async function FollowersPage({
  ...
  // Fetch everyone who follows this profile
  ...
  if (currentUser && followers.length > 0) {
    const ids = followers.map((f: any) => f.id)
    const { data: following } = await supabase
      .from('follows')
      .select('following_id')
      .eq('follower_id', currentUser.id)
      .in('following_id', ids)
    currentUserFollowingIds = (following ?? []).map((f) => f.following_id)
  }
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      <div className="max-w-lg mx-auto px-4 pt-6">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h1 className="font-bold text-ink">Followers</h1>
        ...
        {followers.length === 0 ? (
          <div className="bg-white rounded-card shadow-card p-8 text-center text-gray-500">
            ...
            <p className="font-medium text-ink">No followers yet</p>
          </div>
        ) : (
